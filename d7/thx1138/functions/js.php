...
function thx1138_js_alter(&$js) {
  ...
  if(module_exists('contextual')){
    ...
    drupal_add_js( drupal_get_path('theme', 'thx1138') . '/js/contextual.js' );

  }

  //js from core modules
  if (theme_get_setting('thx1138_js_nuke_module')){
  ...
  //js from contrib
  if (theme_get_setting('thx1138_js_nuke_module_contrib')){
    foreach ($js as $file => $value) {
      if (strpos($file, '/modules/') !== FALSE) {
        unset($js[$file]);
      }
  ...
  if (theme_get_setting('thx1138_js_nuke_misc')){
    foreach ($js as $file => $value) {
  ...
  $js_kill_list = explode("\n", theme_get_setting('thx1138_js_freeform'));

  //grap the css and run through em
  if(theme_get_setting('thx1138_js_freeform')){
    foreach ($js as $file => $value) {
  ...
  if(theme_get_setting('thx1138_js_jquery_latest')){
    if (isset($js['misc/jquery.js'])) {
      $js['misc/jquery.js']['data'] = drupal_get_path('theme', 'thx1138') . '/js/jquery-1.8.2.min.js';
  ...
  if(theme_get_setting('thx1138_js_jquerycdn')){
    $version = theme_get_setting('thx1138_js_jquerycdn_version');
    if (isset($js['misc/jquery.js'])) {
  ...
  if(theme_get_setting('thx1138_js_onefile')){
