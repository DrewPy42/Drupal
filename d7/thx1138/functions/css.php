...
function thx1138_css_alter(&$css) {
  $thx1138_csscore_path = drupal_get_path('theme', 'thx1138') . '/css-drupalcore/';
  $thx1138_cssmodules_path = drupal_get_path('theme', 'thx1138') . '/css-modules/';


  //book
	if( theme_get_setting('thx1138_css_nuke_book') AND module_exists('book') ) {

	  $css = drupal_add_css($thx1138_csscore_path . 'book.admin.css', array('group' => CSS_SYSTEM));
	  $css = drupal_add_css($thx1138_csscore_path . 'book.theme.css', array('group' => CSS_SYSTEM));
  ...
  //theme.css
  if(theme_get_setting('thx1138_css_nuke_theme')){
    foreach ($css as $file => $value) {
  ...
  //admin.css
  if(theme_get_setting('thx1138_css_nuke_admin')){
    foreach ($css as $file => $value) {
  ...
  if(theme_get_setting('thx1138_css_nuke_module_contrib')){
    foreach ($css as $file => $value) {
      //first check those css files we DONT wanna remove
      if (
  ...
  //modules: all modules
  if(theme_get_setting('thx1138_css_nuke_module_all')){
    foreach ($css as $file => $value) {
      //first check those css files we DONT wanna remove
      if (
  ...
  if(theme_get_setting('thx1138_css_nuke_systemtoolbar')){
    foreach ($css as $file => $value) {
      if (
  ...
  //system message folder
  if(theme_get_setting('thx1138_css_nuke_system_message')){
    foreach ($css as $file => $value) {
  ...
  //system menu
  if(theme_get_setting('thx1138_css_nuke_system_menus')){
    foreach ($css as $file => $value) {
  ...
  //system theme
  if(theme_get_setting('thx1138_css_nuke_system_theme')){
    foreach ($css as $file => $value) {
  ...
  //vertical tabs
  if(theme_get_setting('thx1138_css_nuke_vertical')){
    foreach ($css as $file => $value) {
  ...
  $css_kill_list = explode("\n", theme_get_setting('thx1138_css_freeform'));

  //grap the css and run through em
  if(theme_get_setting('thx1138_css_freeform')){
    foreach ($css as $file => $value) {
  ...
  if(theme_get_setting('thx1138_css_onefile')){
    $theme = drupal_get_path('theme', 'thx1138');
