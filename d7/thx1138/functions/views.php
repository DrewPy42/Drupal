...
*/
function thx1138_preprocess_views_view(&$vars){
//    kpr($vars['classes_array']);

  if (theme_get_setting('thx1138_classes_view')) {
    $vars['classes_array'] = array_values(array_diff($vars['classes_array'],array('view')));
  }
  if (theme_get_setting('thx1138_classes_view_name')) {
    $vars['classes_array'] = array_values(array_diff($vars['classes_array'],array('view-'.$vars['name'])));
  }
  if (theme_get_setting('thx1138_classes_view_view_id')) {
...
function thx1138_preprocess_views_view_list(&$vars){
  //we need to go down to the unformatted preprocess to rip out the individual classes
  thx1138_preprocess_views_view_unformatted($vars);
}
...
*/
function thx1138_preprocess_views_view_unformatted(&$vars) {
  //renaming classes
  if(theme_get_setting('thx1138_classes_view_row_rename')){
    $row_first = "first";
    $row_last  = "last";
    $row_count = "count-";
  }else{
  ...
  foreach ($rows as $id => $row) {
    ...
    if (!theme_get_setting('thx1138_classes_view_row')) {
      $vars['classes'][$id][] = 'views-row';
    }
    if (!theme_get_setting('thx1138_classes_view_row_count')) {
    ...
    if (!theme_get_setting('thx1138_classes_view_row_first_last')) {
      if ($count == 1) {
...
/*
function thx1138_preprocess_views_view_field(&$vars) {
 // kpr($vars);
