 ...
 */
$path_thx1138 = drupal_get_path('theme', 'thx1138');

  include_once './' . $path_thx1138 . '/functions/css.php';
  include_once './' . $path_thx1138 . '/functions/js.php';
  include_once './' . $path_thx1138 . '/functions/icons.php';
  include_once './' . $path_thx1138 . '/functions/form.php';
  include_once './' . $path_thx1138 . '/functions/table.php';
  include_once './' . $path_thx1138 . '/functions/views.php';
  include_once './' . $path_thx1138 . '/functions/menu.php';
  include_once './' . $path_thx1138 . '/functions/system.php';
  include_once './' . $path_thx1138 . '/functions/date.php';
  include_once './' . $path_thx1138 . '/functions/misc.php';
  include_once './' . $path_thx1138 . '/functions/forum.php';
  include_once './' . $path_thx1138 . '/functions/blockify.php';
  include_once './' . $path_thx1138 . '/functions/panels.php';
//load in the login
if (theme_get_setting('thx1138_goodies_login')) {
  include_once './' . $path_thx1138 . '/goodies/login.inc';
}

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('thx1138_rebuild_registry')) {
...
*/
function thx1138_preprocess(&$vars, $hook) {
  global $theme;
  global $base_url;
  ...
  $vars['thx1138_poorthemers_helper'] = "";
  ...
  */
  ...
  if ( $hook == "html" ) {
    ...
    $vars['thx1138_path'] = $base_url .'/'. $path_thx1138;


    //lets make it a tiny bit more readable in the html.tpl.php
    //gets processed in thx1138_process_html
...
//    $headers = drupal_get_http_header();
    // if(theme_get_setting('thx1138_404') AND isset($headers['status']) ){
...
function graduate_preprocess(&$vars, $hook) {
    ...
    /*
    if(theme_get_setting('thx1138_403')){
    ...
    */
    ...
    /*
      ...
      thx1138.css - adds css for use with icons & other markup fixes
    */
    if (theme_get_setting('thx1138_css_reset')) {
      drupal_add_css( $path_thx1138 . '/css/reset.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => -20));
    }
    if (theme_get_setting('thx1138_css_reset_html5')) {
      drupal_add_css( $path_thx1138 . '/css/reset-html5.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => -20));
    }
    if (theme_get_setting('thx1138_css_normalize')) {
      drupal_add_css( $path_thx1138 . '/css/normalize.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => -20));
    }
    if (theme_get_setting('thx1138_css_default')) {
      drupal_add_css( $path_thx1138 . '/css/thx1138-default.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => -15));
    }
    if (theme_get_setting('thx1138_css_layout')) {
      drupal_add_css( $path_thx1138 . '/css/thx1138-layout.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => -14));
    }
    if (theme_get_setting('thx1138_css_thx1138styles')) {
      drupal_add_css( $path_thx1138 . '/css/thx1138.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => -10));
    }

    if (theme_get_setting('thx1138_mediaquery_indicator')) {
      drupal_add_css( $path_thx1138 . '/css/thx1138-devel-mediaqueries.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 0));
    }
    ...
    //---html5 fix
    $vars['html5iefix'] = '';
    if(theme_get_setting('thx1138_html5')) {
      $vars['html5iefix'] .= '<!--[if lt IE 9]>';
      $vars['html5iefix'] .= '<script src="' . $path_thx1138 . '/js/html5.js"></script>';
    ...
    if (theme_get_setting('thx1138_classes_body_html')) {
      $vars['classes_array'] = array_values(array_diff($vars['classes_array'],array('html')));
    }

    if (theme_get_setting('thx1138_classes_body_front')) {
    ...
    if (theme_get_setting('thx1138_classes_body_loggedin')) {
    ...
    if (theme_get_setting('thx1138_classes_body_layout')) {
    ...
    if (theme_get_setting('thx1138_classes_body_toolbar')) {
    ...
    if (theme_get_setting('thx1138_test')) {
      $vars['classes_array'][] = "test";
    }

    if(isset($headers['status']) AND theme_get_setting('thx1138_classes_body_status') ){
      $vars['classes_array'][] = "status-". $headers['status'];
    }

    //freeform css class killing
    $remove_class_body = explode(", ", theme_get_setting('thx1138_classes_body_freeform'));
    ...
    if (theme_get_setting('thx1138_expectedmodules')) {
...
//    }
    ...
    if(theme_get_setting('thx1138_frontpage_default_message')){
      unset($vars['page']['content']['system_main']['default_message']);
    }

    // Remove the block template wrapper from the main content block.
    if (theme_get_setting('thx1138_content_block_wrapper') AND
    ...
    */
    ...
    if (theme_get_setting('thx1138_classes_region')) {
    ...
    $vars['id_block'] = "";
    if (theme_get_setting('thx1138_classes_block')) {
    ...
    if (theme_get_setting('thx1138_classes_block_contextual')) {
      $vars['classes_array'] = array_values(array_diff($vars['classes_array'],array('contextual-links-region')));
    }

    if (!theme_get_setting('thx1138_classes_block_id')) {
      $vars['id_block'] = ' id="' . $vars['block_html_id'] . '"';
    }

    if (theme_get_setting('thx1138_classes_block_id_as_class')) {
      $vars['classes_array'][] = $vars['block_html_id'];
    }

    //freeform css class killing
    $remove_class_block = explode(", ", theme_get_setting('thx1138_classes_block_freeform'));
...
//    print_r($vars['classes_array']);
    ...
    if (theme_get_setting('thx1138_classes_node')) {
      $vars['classes_array'] = array_values(array_diff($vars['classes_array'],array('node')));
    }

    if (theme_get_setting('thx1138_classes_node_state')) {
    ...
    */
    ...
    if (theme_get_setting('thx1138_classes_state')) {
    ...
    //freeform css class killing
    $remove_class_node = explode(", ", theme_get_setting('thx1138_classes_node_freeform'));
    $vars['classes_array'] = array_values(array_diff($vars['classes_array'],$remove_class_node));

    // css id for the node
    if (theme_get_setting('thx1138_classes_node_id')) {
    ...
    */
    if (theme_get_setting('thx1138_classes_node_links_inline') AND isset($vars['content']['links']['#attributes']['class'])) {
      $vars['content']['links']['#attributes']['class'] = array_values(array_diff($vars['content']['links']['#attributes']['class'],array('inline')));
    }

    if (theme_get_setting('thx1138_classes_node_links_links') AND (isset($vars['content']['links']['#attributes']['class']))) {
    ...
    if(theme_get_setting('thx1138_poorthemers_helper')){
      $vars['thx1138_poorthemers_helper'] .= " ";
      //foreach ($vars['theme_hook_suggestions'] as $key => $value){
        // $value = str_replace('_','-',$value);
        //$vars['thx1138_poorthemers_helper'] .= "<!-- * " . $value . ".tpl.php -->\n" ;
      //}
   ...
   // kpr($vars['theme_hook_suggestions']);
    ...
    if (theme_get_setting('thx1138_classes_node_links_inline')) {
      $vars['content']['links']['#attributes']['class'] = array_values(array_diff($vars['content']['links']['#attributes']['class'],array('inline')));
    }

  }elseif ( $hook == "field" ) {
    // =======================================| FIELD |========================================
    if (theme_get_setting('thx1138_classes_field_field')) {
...
//--- POOR THEMERS HELPER
  if(theme_get_setting('thx1138_poorthemers_helper')){
    ...
    foreach ($vars['theme_hook_suggestions'] as $key => $value){
        $value = str_replace('_','-',$value);
        $vars['thx1138_poorthemers_helper'] .= "<!-- tpl file: * " . $value . ".tpl.php -->\n" ;
    }
    // $vars['thx1138_poorthemers_helper'] .= "[*]file:" . $vars['template_file'];
    $vars['thx1138_poorthemers_helper'] .= "";
  }else{
    $vars['thx1138_poorthemers_helper'] ="";
  }
...
function thx1138_process_html(&$vars, $hook) {
...
*/
function thx1138_process_html_tag(&$vars) {
  $el = &$vars['element'];
...
*/
function thx1138_class_killer(&$vars){
  $remove_class_node = explode(", ", theme_get_setting('thx1138_classes_node_freeform'));
  $vars['classes_array'] = array_values(array_diff($vars['classes_array'],$remove_class_node));
  $vars['classes'] = "";
 ...
 */
function thx1138_theme_registry_alter(&$theme_registry) {
//  kpr($theme_registry);
  //enough of this bull lets kill em classes
  $theme_registry['node']['preprocess functions'][] = 'thx1138_class_killer';
